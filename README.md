...
- **高性能低占用** — Tauri 原生窗口，内存占用极低（安装包约 15-20MB，远小于 Electron）
...
GitHub Releases 提供预构建安装包（由 GitHub Actions 自动构建）：
...
| 工具名 | 功能 |
|--------|------|
| `llamaui_start` | 启动模型实例（支持指定名称、模型、模式、GPU 层数等；未配置 llama.cpp 路径时自动检测；端口可选，默认自动选择空闲端口） |
| `llamaui_stop` | 停止指定实例 |
