...
llama.cpp 桌面管理工具 — 为本地大模型提供可视化启动、模型管理和配置预设界面。

支持 Claude Code / Codex CLI 工具调用（MCP Server）。

## 功能

- **一键启动** — 支持 llama-server (HTTP API) 和 llama-cli 双模式
- **实时日志** — stdout/stderr 流式输出，终端风格显示
...
| 工具名 | 功能 |
|--------|------|
| `llamaui_start` | 启动模型 (支持指定模型、模式、GPU 层数等) |
| `llamaui_stop` | 停止当前运行的模型 |
| `llamaui_status` | 查询运行状态 |
